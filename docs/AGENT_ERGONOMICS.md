...
When an agent is navigating the codebase, it MUST prioritize reading `ANTIGRAVITY.md` files in the current directory before reading implementation code.

- **Local Context:** Every major feature directory (e.g., `app/recipes/`, `lib/pantry/`) must contain a `ANTIGRAVITY.md` file defining its specific invariants, technical decisions, and testing requirements.
...
Culinary logic (e.g., unit categories, macro calculation rules) is documented in the `ANTIGRAVITY.md` files within the `app/recipes/` and `lib/` directories. Agents must verify culinary assumptions against these local files rather than relying on general model knowledge.
