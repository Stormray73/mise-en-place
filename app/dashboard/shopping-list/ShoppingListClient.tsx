...
export default function ShoppingListClient({
  ...
  const handleDeleteStore = async (id: string) => {
    if (!confirm("Are you sure you want to delete this store? Items assigned to it will become unassigned.")) return;
  ...
  const handleCompleteShop = async () => {
    const checkedList = filteredList.filter((item) => checkedItems[getItemKey(item)]);
    ...
    const payload = checkedList.map((item) => {
      ...
      const qty = actualQuantities[key] !== undefined ? actualQuantities[key] : item.neededQuantity;
      return {
        id: item.id,
    ...
    const activeStoreId = selectedStoreTab === "all" || selectedStoreTab === "unassigned" ? null : selectedStoreTab;
  ...
  const activeCheckedCount = filteredList.filter((item) => checkedItems[getItemKey(item)]).length;
    ...
    <div className="space-y-8 pb-24">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-zinc-900/60 p-4 rounded-xl border border-zinc-800 backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={handleCurrentWeek} className="border border-zinc-700 bg-zinc-800 hover:bg-zinc-700">
            Current Week
          </Button>
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            className={isShoppingMode ? "border border-red-500/50 hover:bg-red-500/10 text-red-400" : "bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-md shadow-blue-500/10 font-bold"}
          >
      ...
      {showManageStores && (
        <Card className="p-6 border border-zinc-800 bg-zinc-900/40 backdrop-blur-md animate-in slide-in-from-top duration-300">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold text-zinc-100">Store Management</h3>
            <button
            ...
            <form onSubmit={handleCreateStore} className="space-y-4">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="flex-1"
                />
                <Button type="submit" disabled={isCreatingStore || !newStoreName.trim()}>
            ...
            <div className="space-y-2">
              ...
              <div className="max-h-40 overflow-y-auto space-y-2 pr-2">
                {stores.map((store) => (
                  ...
                  >
                    <span className="text-sm font-medium text-zinc-200">{store.name}</span>
                    <button
                      type="button"
                ...
                {stores.length === 0 && (
                  <p className="text-xs text-zinc-500 italic">No stores created yet.</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="p-6 lg:col-span-1 h-fit border border-zinc-800 bg-zinc-900/30">
          <h2 className="text-lg font-bold mb-4 text-zinc-200">Filters & Range</h2>
        ...
        <Card className="p-6 lg:col-span-2 border border-zinc-800 bg-zinc-900/30">
          <h2 className="text-lg font-bold mb-4 text-zinc-200">Add Custom Item</h2>
      ...
      <div className="border-b border-zinc-800 flex gap-2 overflow-x-auto pb-px">
        ...
        {stores.map((store) => {
          const count = initialList.filter((item) => item.storeId === store.id).length;
          return (
            <button
              key={store.id}
      ...
      <div className="space-y-8">
            ...
            <div key={dept} className="space-y-3">
              ...
              <div className="space-y-3">
                {deptItems.map((item) => {
                  ...
                  const currentQty = actualQuantities[key] !== undefined ? actualQuantities[key] : item.neededQuantity;
                  ...
                  return (
                    ...
                    >
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <h4
                              className={`font-bold text-base text-zinc-100 ${
                                isShoppingMode && isChecked ? "line-through text-zinc-500" : ""
                              }`}
                            >
                              {item.name}
                            </h4>
                          ...
                          <p className="text-xs text-zinc-500 mt-0.5">
                            {item.reason === "meal-plan" ? (
                              <>
                                ...
                                (Pantry stock: {item.availableQuantity} {item.unit})
                              </>
                      ...
                      <div className="flex items-center gap-4 ml-4 shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                          {isShoppingMode ? (
                            <div className="flex items-center bg-zinc-800 border border-zinc-700 rounded-md overflow-hidden">
                              <button
                                type="button"
                                onClick={() => handleQuantityChange(key, currentQty - 1)}
                        ...
                        {!isShoppingMode && item.reason === "manual" && item.id && (
                          <button
                            type="button"
                            onClick={() => {
        ...
        {filteredList.length === 0 && (
          <div className="text-center py-12 text-zinc-500 bg-zinc-900/20 rounded-xl border border-dashed border-zinc-800">
            No items in this category. Either you have everything you need, or you haven&apos;t planned any meals for this range or store.
          </div>
        )}
      </div>
        ...
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 w-11/12 max-w-xl bg-zinc-900 border border-emerald-500/30 shadow-xl shadow-emerald-500/5 p-4 rounded-xl flex items-center justify-between gap-4 z-50 animate-bounce-subtle backdrop-blur-lg">
          <div>
            <div className="text-sm font-bold text-zinc-200">
              🛒 Shopping ({selectedStoreTab === "all" ? "All Stores" : stores.find(s => s.id === selectedStoreTab)?.name || "Unassigned"})
            </div>
          ...
          <div className="flex gap-2">
            ...
            >
              {isCompleting ? "Saving..." : `Complete Shop (${activeCheckedCount})`}
            </Button>
          </div>
        </div>
      )}
