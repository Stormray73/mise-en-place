...
Before introducing any new npm package, you MUST invoke the `scan_dependencies` skill (from the installed SecureCoder plugin). Do not add any import or update `package.json` until the dependency is confirmed safe.

### 3. Implementation Loop (Per Story/Step)
...
### 5. Documentation & Context Persistence
...
Delegate peer review to a **fresh, independent agent** via `invoke_subagent` using the `peer-reviewer` skill. Do not perform a self-review. Pass the list of modified files and the story AC as context. The subagent will write its findings to its own `walkthrough.md` artifact — read the outcome from the subagent's return message and resolve any Critical or Major findings before marking the story complete.

### 7. Handoff Summary

Create or update the `walkthrough.md` artifact with a summary of all changes made, tests added, and the peer review outcome. This serves as a record for the user and any orchestrating agent.
