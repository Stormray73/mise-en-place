...
4. **Decompose Epics:** For any NEW epics created in `v1/`, activate the `feature-decomposer` skill **inline** (same agent context — no subagent needed, as triage and decomposition are a single continuous planning session). The `feature-decomposer` skill will present its output in an `implementation_plan.md` artifact for user approval before writing to disk.
...
If you want feedback triage to run automatically on a schedule (e.g., to process a `FEEDBACK_BACKLOG.md` that gets updated externally), ask the user to use the `/schedule` slash command. For example: "Process the feedback backlog every Monday morning."
