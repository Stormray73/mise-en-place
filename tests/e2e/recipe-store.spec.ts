...
test.describe("Recipe Store User Journeys", () => {
  ...
  test("Story 1: Create a new recipe with USDA ingredients", async ({
    ...
    await page.route("**/api/usda/search?q=Tomato*", async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page.getByLabel(/yield unit/i).fill("Litre");
    ...
    await expect(
      page.locator('input[value*="Tomatoes, red, ripe"]'),
    ).toBeVisible();
    ...
    // Verify redirect to dashboard and recipe visibility
  ...
  test("Story 5: Edit recipe", async ({ page }) => {
    ...
    // Change title
    await page.getByLabel(/recipe title/i).fill("Updated Marinara");
    await page.getByRole("button", { name: /save recipe/i }).click();
